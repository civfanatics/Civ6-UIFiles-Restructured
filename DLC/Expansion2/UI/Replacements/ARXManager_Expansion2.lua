...
function PopulateVictoryType(victoryType:string, typeText:string)
	...
	elseif (victoryType == "VICTORY_DIPLOMATIC") then
		firstTiebreakerFunction = function(p)
			return p:GetStats():GetDiplomaticVictoryPoints();
		end;
...
function DrawVictoryProgress()
	...
	if(Game.IsVictoryEnabled("VICTORY_DIPLOMATIC")) then
		PopulateVictoryType("VICTORY_DIPLOMATIC", "<img src=Victory_Time.png align=left>" .. Locale.Lookup("LOC_DIPLOMATIC").."<br>");
        bShownAType = true;
	end

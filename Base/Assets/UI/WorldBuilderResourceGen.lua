...
------------------------------------------------------------------------------
WorldBuilderResourceGenerator = {};
------------------------------------------------------------------------------
function WorldBuilderResourceGenerator.Create(args)

	print ("In WorldBuilderWorldBuilderResourceGenerator.Create()");
	...
	local instance = {
		...
		-- methods
		__InitResourceData		= WorldBuilderResourceGenerator.__InitResourceData,
		__FindValidLocs			= WorldBuilderResourceGenerator.__FindValidLocs,
		__GetLuxuryResources	= WorldBuilderResourceGenerator.__GetLuxuryResources,
		__ValidLuxuryPlots		= WorldBuilderResourceGenerator.__ValidLuxuryPlots,
		__PlaceLuxuryResources		= WorldBuilderResourceGenerator.__PlaceLuxuryResources,
		__ScoreLuxuryPlots			= WorldBuilderResourceGenerator.__ScoreLuxuryPlots,
		__GetWaterLuxuryResources			= WorldBuilderResourceGenerator.__GetWaterLuxuryResources,
		__SetWaterLuxury			= WorldBuilderResourceGenerator.__SetWaterLuxury,
		__PlaceWaterLuxury			= WorldBuilderResourceGenerator.__PlaceWaterLuxury,
		__GetStrategicResources	= WorldBuilderResourceGenerator.__GetStrategicResources,
		__ValidStrategicPlots		= WorldBuilderResourceGenerator.__ValidStrategicPlots,
		__ScoreStrategicPlots			= WorldBuilderResourceGenerator.__ScoreStrategicPlots,
		__PlaceStrategicResources		= WorldBuilderResourceGenerator.__PlaceStrategicResources,
		__GetWaterStrategicResources	= WorldBuilderResourceGenerator.__GetWaterStrategicResources,
		__SetWaterStrategic			= WorldBuilderResourceGenerator.__SetWaterStrategic,
		__PlaceWaterStrategic			= WorldBuilderResourceGenerator.__PlaceWaterStrategic,
		__GetOtherResources		= WorldBuilderResourceGenerator.__GetOtherResources,
		__PlaceOtherResources		= WorldBuilderResourceGenerator.__PlaceOtherResources,
		__GetWaterOtherResources		= WorldBuilderResourceGenerator.__GetWaterOtherResources,
		__PlaceWaterOtherResources		= WorldBuilderResourceGenerator.__PlaceWaterOtherResources,
		__RemoveOtherDuplicateResources		= WorldBuilderResourceGenerator.__RemoveOtherDuplicateResources,
		__RemoveDuplicateResources		= WorldBuilderResourceGenerator.__RemoveDuplicateResources,
		__ScorePlots			= WorldBuilderResourceGenerator.__ScorePlots,
		__ScoreWaterPlots			= WorldBuilderResourceGenerator.__ScoreWaterPlots,
...
------------------------------------------------------------------------------
function WorldBuilderResourceGenerator:__InitResourceData()
...
------------------------------------------------------------------------------
function WorldBuilderResourceGenerator:__GetLuxuryResources()
...
------------------------------------------------------------------------------
function WorldBuilderResourceGenerator:__ValidLuxuryPlots(eContinent)
...
------------------------------------------------------------------------------
function WorldBuilderResourceGenerator:__PlaceLuxuryResources(eChosenLux, eContinent)
	...
	for iI = 1, iNumToPlace do
			if (iI <= #self.aaPossibleLuxLocs[eChosenLux]) then
				...
				WorldBuilder.MapManager():SetResourceType(pPlot, self.aIndex[eChosenLux], 1);
...
------------------------------------------------------------------------------
function WorldBuilderResourceGenerator:__ScoreLuxuryPlots(iResourceIndex, eContinent)
...
------------------------------------------------------------------------------
function WorldBuilderResourceGenerator:__GetWaterLuxuryResources()
...
------------------------------------------------------------------------------
function WorldBuilderResourceGenerator:__SetWaterLuxury(eChosenLux, latitudeMax, latitudeMin, index)
...
------------------------------------------------------------------------------
function WorldBuilderResourceGenerator:__PlaceWaterLuxury(eChosenLux, pPlot)
...
------------------------------------------------------------------------------
function WorldBuilderResourceGenerator:__GetStrategicResources()
	local continentsInUse = Map.GetContinentsInUse();	
...
------------------------------------------------------------------------------
function WorldBuilderResourceGenerator:__ValidStrategicPlots(iWeight, eContinent)
...
------------------------------------------------------------------------------
function WorldBuilderResourceGenerator:__PlaceStrategicResources(eContinent)
	-- Go through continent placing the chosen strategic
	for i, row in ipairs(self.aResourcePlacementOrderStrategic) do
		local eResourceType = self.aIndex[row.ResourceIndex];
		...
		for iI = 1, iNumToPlace do
			if (iI <= #self.aaPossibleStratLocs[row.ResourceIndex]) then
				...
				-- Place at this location
				local pPlot = Map.GetPlotByIndex(iMapIndex);
				WorldBuilder.MapManager():SetResourceType(pPlot, eResourceType, 1);
--				print ("   Placed at (" .. tostring(pPlot:GetX()) .. ", " .. tostring(pPlot:GetY()) .. ") with score of " .. tostring(iScore));
			end
		end
	end
...
------------------------------------------------------------------------------
function WorldBuilderResourceGenerator:__ScoreStrategicPlots(iResourceIndex, eContinent)
	-- Clear all earlier entries (some might not be valid if resources have been placed
...
------------------------------------------------------------------------------
function WorldBuilderResourceGenerator:__GetWaterStrategicResources()
...
------------------------------------------------------------------------------
function WorldBuilderResourceGenerator:__SetWaterStrategic(eChosenStrat, latitudeMax, latitudeMin, index)
	local bOddSwitch = false;
	local bFirst = true;
	local iNumber = 0
	local iW, iH;
...
------------------------------------------------------------------------------
function WorldBuilderResourceGenerator:__PlaceWaterStrategic(eChosenStrat, pPlot)
	if (ResourceBuilder.CanHaveResource(pPlot, self.eResourceType[eChosenStrat])) then
		...
		if(score >= 85 + 5 * self.iOccurencesPerFrequency) then
			WorldBuilder.MapManager():SetResourceType(pPlot, self.aIndex[eChosenStrat], 1);
			return true
		end
	end
...
------------------------------------------------------------------------------
function WorldBuilderResourceGenerator:__GetOtherResources()
	self.aOtherType = {};
...
------------------------------------------------------------------------------
function WorldBuilderResourceGenerator:__PlaceOtherResources()
...
------------------------------------------------------------------------------
function WorldBuilderResourceGenerator:__ScorePlots(iResourceIndex)
...
------------------------------------------------------------------------------
function WorldBuilderResourceGenerator:__GetWaterOtherResources()
...
------------------------------------------------------------------------------
function WorldBuilderResourceGenerator:__PlaceWaterOtherResources()
...
------------------------------------------------------------------------------
function WorldBuilderResourceGenerator:__ScoreWaterPlots(iResourceIndex)
...
------------------------------------------------------------------------------
function WorldBuilderResourceGenerator:__RemoveOtherDuplicateResources()
					...
					if (self.eResourceClassType[index] ~= "RESOURCECLASS_STRATEGIC" and self.eResourceClassType[index] ~= "RESOURCECLASS_LUXURY" and self.eResourceClassType[index] ~= "RESOURCECLASS_ARTIFACT") then
						if(self.eResourceType[index]  == pPlot:GetResourceTypeHash()) then
							...
							if(bRemove == true) then
								WorldBuilder.MapManager():SetResourceType(pPlot, NO_RESOURCE);
							end
						end
					end		
				end
...
------------------------------------------------------------------------------
function WorldBuilderResourceGenerator:__RemoveDuplicateResources(plot, eResourceType)
	local iCount = 0;
